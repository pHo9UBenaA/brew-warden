...
func decodeObject(data []byte, out any, keys ...string) error {
	if len(data) > maxResponse || !utf8.Valid(data) {
		return errors.New("provenance response exceeds bounds")
	}
	...
	if _, err := decoder.Token(); err != io.EOF {
		return errors.New("trailing provenance JSON")
	}
	var fields map[string]json.RawMessage
	if err := json.Unmarshal(data, &fields); err != nil || fields == nil {
		return errors.New("provenance response is not an object")
	}
	for name, value := range fields {
		for _, key := range keys {
			if strings.EqualFold(name, key) && (name != key || bytes.Equal(bytes.TrimSpace(value), []byte("null"))) {
				return errors.New("ambiguous or null provenance field")
			}
		}
	}
	if err := json.Unmarshal(data, out); err != nil {
		return errors.New("invalid provenance field type")
	}
	return nil
}
...
func validateJSON(d *json.Decoder, depth int) error {
	if depth > 16 {
		return errors.New("provenance nesting exceeds limit")
	}
	token, err := d.Token()
	if err != nil {
		return errors.New("malformed provenance JSON")
	}
	switch token {
	case json.Delim('{'):
		...
		for d.More() {
			...
			if err != nil || !ok || seen[name] {
				return errors.New("duplicate or invalid provenance key")
			}
			seen[name] = true
	...
	default:
		if _, delimiter := token.(json.Delim); delimiter {
			return errors.New("unexpected provenance delimiter")
		}
		return nil
	}

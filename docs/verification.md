...
| `./scripts/check.sh fuzz` / `task fuzz` | Commit-message, strict configuration, attempt-record, publication-response and advisory-record parser fuzzing; `FUZZTIME` defaults to 10s per target | None required |
